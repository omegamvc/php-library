...
use System\Database\Query\Join\InnerJoin;
...
abstract class AbstractCrud implements CrudInterface
{
    ...
    public function read(): bool
    {
        ...
        $read = Query::from($this->TABLE_NAME, $this->PDO)
            ->select($arr_column)
            ->equal($key, $value)
            ->single()
        ;
    ...
    {
        $create = Query::from($this->TABLE_NAME, $this->PDO)
            ->insert()
            ->values($this->COLUMNS)
            ->execute()
        ;
    ...
    public function delete(): bool
    {
        ...
        return Query::from($this->TABLE_NAME, $this->PDO)
            ->delete()
            ->equal($key, $value)
            ->execute()
        ;
    ...
    public function isExist(): bool
    {
        ...
        $get = Query::from($this->TABLE_NAME, $this->PDO)
            ->select(['id'])
            ->equal($key, $value)
            ->single()
        ;
    ...
    {
        $table_info = Query::from($this->TABLE_NAME, $this->PDO)
            ->info()
        ;
    ...
    {
        $ref = Query::from($this->TABLE_NAME, $this->PDO)
            ->select([$table . '.*'])
