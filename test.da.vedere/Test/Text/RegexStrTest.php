...
final class RegexStrTest extends TestCase
{
    ...
    {
        $res = Str::isMatch('adriano@mail.com', Regex::EMAIL, 'cek email');
        $this->assertTrue($res);

        $res = Str::isMatch('adriano.com', Regex::EMAIL, 'cek email');
        $this->assertFalse($res);
    }

    public function testRegexUsername()
    {
        $res = Str::isMatch('adriano', Regex::USER, 'cek user');
        $this->assertTrue($res);

        $res = Str::isMatch('1adriano', Regex::USER, 'cek user');
        $this->assertFalse($res);
